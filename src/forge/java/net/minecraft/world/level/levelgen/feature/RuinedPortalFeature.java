...
public class RuinedPortalFeature extends StructureFeature<RuinedPortalConfiguration> {
   ...
   static boolean isCold(BlockPos pPos, Biome pBiome) {
      return pBiome.getTemperature(pPos) < 0.15F;
   }

   static int findSuitableY(Random pRandom, ChunkGenerator pChunkGenerator, RuinedPortalPiece.VerticalPlacement pVerticalPlacement, boolean pAirPocket, int pBaseHeight, int pYSpan, BoundingBox pBoundingBox, LevelHeightAccessor pLevel) {
      int i;
      if (pVerticalPlacement == RuinedPortalPiece.VerticalPlacement.IN_NETHER) {
         ...
         int j = pBaseHeight - pYSpan;
         i = getRandomWithinInterval(pRandom, 70, j);
      } else if (pVerticalPlacement == RuinedPortalPiece.VerticalPlacement.UNDERGROUND) {
         int i1 = pBaseHeight - pYSpan;
         i = getRandomWithinInterval(pRandom, 15, i1);
      ...
      List<NoiseColumn> list = list1.stream().map((p_160270_) -> {
         return pChunkGenerator.getBaseColumn(p_160270_.getX(), p_160270_.getZ(), pLevel);
